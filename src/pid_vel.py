...
class PidVelocity():
    def __init__(self):
        ...
        self.target = 0
        self.motor = 0 # Don't need this! Motor signals will come from arduino
        self.vel = 0
        self.integral = 0
        self.error = 0
        self.derivative = 0
        ...
        ### get parameters ####
        self.Kp = rospy.get_param('~Kp',10) # Adjustable gain!
        self.Ki = rospy.get_param('~Ki',10) # Adjustable gain!
        self.Kd = rospy.get_param('~Kd',0.001) # Adjustable gain!
        self.out_min = rospy.get_param('~out_min',-255) # May not need this. This is probably for the motor.
        self.out_max = rospy.get_param('~out_max',255) # May not need this. This is probably for the motor.
